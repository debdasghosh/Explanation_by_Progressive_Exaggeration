...
def load_images_and_labels(imgs_names, image_dir, n_class, attr_list, input_size=128, num_channel=3, do_center_crop=False):
    imgs = np.zeros((imgs_names.shape[0], num_channel, input_size, input_size), dtype=np.float32)
    ...
    for i, img_name in tqdm(enumerate(imgs_names)):
        ...
        img = np.reshape(img, [num_channel,input_size,input_size])
        img = img / 255.0 
        img = img - 0.5
        img = img * 2.0
