...
def main():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for batch_idx, (data, target) in enumerate(train_loader, 0):
            ...
            pseudo_y = utils_sngan.sample_pseudo_labels(num_classes, data.shape[0], device)
            #print(pseudo_y)
            #print(target[:,0])
            if (batch_idx) % 5 == 0:
                noise, lables = get_noise(input_size,data.shape[0],device,num_classes,gen)
                #print(noise)
                fake, fake_embedding = gen(data, lables) 
                dis_fake = dis(fake, lables)
                dis_real = dis(data, target)
                loss_gen = distance(dis_fake, dis_real)
                ...
                writer.add_scalar('Generator Loss', loss_gen.item(), counter)

            noise, lables = get_noise(input_size,data.shape[0],device,num_classes,gen)
            fake, fake_embedding = gen(data, lables ) 
            dis_fake = dis(fake, target ) 
            dis_real = dis(data, lables ) 
            ...
            writer.add_scalar('Discriminator Loss', dis_loss, counter)

            print('batch id: {}, Discriminator Loss: {}, Generator Loss: {}'.format(batch_idx, dis_loss, loss_gen.item() ))
            
            if counter % 400 == 0:
                real = data
                fake_path = os.path.join(
                    './SNGAN',
                    'fake_{}_iter_{:07d}.png'.format(batch_idx, epoch)
                )
                real_path = os.path.join(
                    './SNGAN',
                    'real_{}_iter_{:07d}.png'.format(batch_idx, epoch)
                )
        ...
        '''
        utils_sngan.save_images(
                epoch, epoch // EPOCHS, './SNGAN',
                './SNGAN/preview', fake, data
            )
        '''
        # Save checkpoints!
        save_checkpoints(
            './SNGAN', epoch, epoch // EPOCHS,
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(train_loader, 0):
            pseudo_y = utils_sngan.sample_pseudo_labels(num_classes, BATCH_SIZE, device)
            data, target = data.cuda(), target.cuda()
            data, target = Variable(data), Variable(target)
            fake, fake_embedding = gen(data, pseudo_y )
            break
