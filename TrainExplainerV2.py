...
for epoch in range(1, EPOCHS + 1):
    ...
    for batch_idx, (data, target) in enumerate(train_loader, 0):
        ...
        with torch.no_grad():
            real_img_cls_prediction = torch.sigmoid(pretrained_classifier(x_source.clone().detach()))
            fake_img_cls_prediction = torch.sigmoid(pretrained_classifier(fake_target_img.clone().detach()))
            real_img_recons_cls_prediction = torch.sigmoid(pretrained_classifier(fake_source_img.clone().detach()))
        
        # ============= pre-trained classifier loss =============
        y_target_flt = y_target.type(torch.cuda.FloatTensor)
        real_p = y_target_flt*0.1 
        fake_q = fake_img_cls_prediction[:,target_class] 
        fake_evaluation = (real_p * torch.log(fake_q) ) + ( (1-real_p) * torch.log(1-fake_q) )
        fake_evaluation = -fake_evaluation.mean()
        
        recons_evaluation = (real_img_cls_prediction[:,target_class] * torch.log(real_img_recons_cls_prediction[:,target_class]) ) + ( (1-real_img_cls_prediction[:,target_class]) * torch.log(1-real_img_recons_cls_prediction[:,target_class]) )
        recons_evaluation = -recons_evaluation.mean() 
        ...
        if counter % 400 == 0:
            ...
            writer.add_scalar('fake_evaluation', fake_evaluation)
            writer.add_scalar('recons_evaluation', recons_evaluation)
